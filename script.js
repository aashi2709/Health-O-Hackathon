...
const workshops = [
    { 
        ...
        type: "offline- Aditya Birla hospital, Vijay Nagar, Indore",
    ...
    { 
        ...
        type: "offline- Apollo hospital, AB Road, Indore",
...
function showModal(workshop) {
    ...
    // Add duration
    document.getElementById("modal-duration").textContent = `⏱️ Duration: ${workshop.duration}`;
...
function saveRegistrationData(name, email, workshop) {
    let registrations = JSON.parse(localStorage.getItem("registrations")) || [];
    registrations.push({ name, email, workshop });
...
document.getElementById("registration-form").addEventListener("submit", function(event) {
    ...
    const workshop = document.getElementById("event").value;
    ...
    fetch("https://script.google.com/macros/s/AKfycbx6nyUaWEa90sVFuUIvYeMgsY1G7yGav8IGYDtlsRa4GAmtPvS1Fye5X2ZkEOrJx-Zm/exec", {
        method: "POST",
...
function calculateScore() {
    ...
    // Debugging: Check if the result div exists
    if (!resultDiv) {
        console.error("Error: Result div (#result) not found!");
        return;
    }

    if (unanswered > 0) {
        resultDiv.innerHTML = `<p class="warning">⚠️ You missed ${unanswered} question(s). Please answer all questions to get an accurate result.</p>`;
    ...
    if (totalScore >= 0 && totalScore <= 5) {
        message = "🌿 Your mental health is in great shape! Keep up the good self-care habits.";
    } else if (totalScore >= 6 && totalScore <= 12) {
        message = "🙂 You might be experiencing mild stress or anxiety. Consider taking short breaks and engaging in relaxing activities.";
    } else if (totalScore >= 13 && totalScore <= 20) {
        message = "😟 Your score suggests moderate mental health challenges. Try mindfulness exercises or talk to a trusted friend.";
    } else {
        message = "🚨 Your score indicates high levels of stress or emotional distress. It's important to seek support from a mental health professional.";
    }

    // Show result
    resultDiv.innerHTML = `
        <div class="result-box">
            <h2>Your Mental Health Score: ${totalScore}</h2>
            <p>${message}</p>
        </div>
    `;
    resultDiv.style.color = "#333";

    // Debugging: Confirm the function ran
